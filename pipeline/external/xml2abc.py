    ...
    def outVoices (s, maatrecs, ip, isSib): # output all voices of part ip
        ...
        for iv in vnum_keys:
            ...
            for im in range (len (s.gMaten)):
                ...
                vn.append (outVoice (measure, divs [im], im, ip, unitL, s.rbm, metra [im]))
...
def outVoice (measure, divs, im, ip, unitL, rebeam, metrum):    # note/elem objects of one measure in one voice
    ix = 0
    ...
    def doDirection (s, e, i, es): # parse a musicXML direction tag
        ...
        def startStop (dtype, vs, stfnum=1):
            ...
            if type in typmap:                      # opening the direction
                ...
                # Just ignore unknown types instead of crashing
        ...
        for dirtyp in dirtypes:
            ...
            if metr != None:
                ...
                # Ensure we match actual numbers (at least one digit)
                tmpro = re.search (r'\b\d+(?:\.\d*)?|\.\d+\b', metr.findtext ('per-minute') or '')
